...
function initRequestForm() {
  ...
  form.addEventListener('submit', function(e) {
    ...
    const data = {
      ...
      rhesus: document.getElementById('reqRhesus')?.value,
      bags: document.getElementById('reqBags')?.value,
    ...
    // Add to dummy lists
    dummyRequests.unshift(data);
    myRequests.unshift(data);

    showModal('modalRequestSuccess');
    form.reset();
  });
}
  ...
  if (requests.length === 0) {
    container.innerHTML = `
      <div class="empty-state">
        ...
        <a href="permintaan-darurat.html" class="btn btn-primary mt-16">Buat Permintaan</a>
      </div>`;
    return;
  }
...
function getMatchedRequests() {
  ...
  return dummyRequests.filter(r =>
    r.bloodType === currentUser.bloodType &&
    r.rhesus === currentUser.rhesus &&
    (r.status === 'Menunggu Donor' || r.status === 'Dihubungi') &&
  ...
  if (!currentUser.isDonorActive) {
    container.innerHTML = `
      <div class="empty-state" style="grid-column:1/-1">
        ...
        <a href="profil-donor.html" class="btn btn-primary mt-16">Lengkapi Profil Donor</a>
      </div>`;
    return;
  }
...
function confirmWilling() {
  ...
  // TODO: Replace with Laravel API POST /api/donor-commitments
  const req = dummyRequests.find(r => r.id === pendingWillingId);
  if (req) {
    req.status = 'Ada Pendonor';
    req.matchedDonor = currentUser.name;
  }
  ...
  const histEntry = {
    id: Date.now(),
    date: new Date().toLocaleDateString('id-ID', { day: 'numeric', month: 'short', year: 'numeric' }),
    patient: req?.patient || 'Pasien',
    hospital: req?.hospital || 'Rumah Sakit',
    location: req?.location || '-',
    bloodType: `${currentUser.bloodType}${currentUser.rhesus}`,
    status: 'Menunggu Konfirmasi Selesai',
    notes: 'Bersedia membantu donor'
  };
  myDonorHistory.unshift(histEntry);

  acceptedRequests.add(pendingWillingId);
  pendingWillingId = null;
  hideModal('modalWilling');
  renderMatchedRequests();
  showToast('Terima kasih! Anda telah bersedia membantu donor darah. 🩸', 'success');
}
  ...
  if (myDonorHistory.length === 0) {
    container.innerHTML = `
      <div class="empty-state">
        ...
        <a href="profil-donor.html" class="btn btn-primary mt-16">Aktifkan Profil Donor</a>
      </div>`;
    return;
  }
...
function initLoginForm() {
  ...
  if (loginForm) {
    loginForm.addEventListener('submit', function(e) {
      e.preventDefault();
      // TODO: Replace with Laravel API POST /api/auth/login
      // Simulate login - save auth state
      authLogin({
        name: 'Ahmad Fauzi',
        email: 'ahmad.fauzi@email.com',
        avatar: 'AF',
      ...
      showModal('modalAuthSuccess');
      document.getElementById('authSuccessMsg').textContent = 'Login berhasil! Selamat datang kembali.';
    });
  }
  ...
  if (registerForm) {
    registerForm.addEventListener('submit', function(e) {
      e.preventDefault();
      // TODO: Replace with Laravel API POST /api/auth/register
      const nameEl = registerForm.querySelector('input[placeholder="Nama lengkap"]');
      const emailEl = registerForm.querySelector('input[type="email"]');
      const name = nameEl ? nameEl.value : 'Pengguna Baru';
      const email = emailEl ? emailEl.value : '';
      const initials = name.split(' ').map(w => w[0]).join('').substring(0, 2).toUpperCase();
      authLogin({ name, email, avatar: initials, bloodType: '-', location: '-' });
      showModal('modalAuthSuccess');
      document.getElementById('authSuccessMsg').textContent = 'Akun berhasil dibuat! Selamat bergabung dengan BloodConnect Aceh.';
    });
  }
}
...
function initAuthUI() {
  ...
  if (sidebarUserEl && !isAdminPage) {
    if (loggedIn) {
      sidebarUserEl.setAttribute('onclick', "window.location='profil-donor.html'");
      ...
      sidebarUserEl.setAttribute('onclick', "window.location='login.html'");
      sidebarUserEl.innerHTML = `
        <div class="avatar" style="background:var(--gray-300);color:var(--gray-600)">?</div>
        <div class="sidebar-user-info">
          <div class="sidebar-user-name">Tamu</div>
          <div class="sidebar-user-role"><a href="login.html" style="color:var(--primary);font-weight:600">Masuk / Daftar →</a></div>
        </div>`;
    }
  }
  ...
  if (welcomeTitle && welcomeSubtitle && welcomeActions) {
    if (loggedIn) {
      ...
      welcomeActions.innerHTML = `
        <a href="cari-donor.html" class="btn btn-white btn-sm">🔍 Cari Donor</a>
        <a href="permintaan-darurat.html" class="btn btn-sm" style="background:rgba(255,255,255,0.2);color:white;border:1px solid rgba(255,255,255,0.4)">🚨 Ajukan Permintaan</a>
        <a href="permintaan-cocok.html" class="btn btn-sm" style="background:rgba(255,255,255,0.2);color:white;border:1px solid rgba(255,255,255,0.4)">🩸 Permintaan Cocok</a>`;
    }
  }
  ...
  if (loggedIn && loginBanner && statsGrid) {
    ...
    statsGrid.innerHTML = `
      <div class="stat-card">
        ...
        <div class="stat-change up">↑ 1 baru hari ini</div>
      </div>
      <div class="stat-card">
      ...
      <div class="stat-card">
        ...
        <div class="stat-change up" style="color:var(--success)">● Tersedia</div>
      </div>
      <div class="stat-card">
        <div class="stat-icon yellow">📅</div>
        <div class="stat-value" style="font-size:1.2rem">15 Mar</div>
        <div class="stat-label">Donor Berikutnya</div>
        <div class="stat-change" style="color:var(--warning)">91 hari lagi</div>
      </div>`;
      ...
      grid2.innerHTML = `
        <div>
          <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:16px">
            ...
            <a href="permintaan-cocok.html" class="text-sm text-primary font-semibold">Lihat Semua →</a>
          </div>
          <div style="display:flex;flex-direction:column;gap:12px">
            <div class="request-card urgency-darurat">
              ...
              <div style="display:flex;gap:8px;flex-wrap:wrap">
                <a href="permintaan-cocok.html" class="btn btn-primary btn-sm">🩸 Bersedia Donor</a>
            ...
            <div class="request-card" style="border-left:3px solid var(--primary)">
              ...
              <div style="display:flex;gap:8px">
                <a href="riwayat-permintaan.html" class="btn btn-ghost btn-sm">Lihat Riwayat</a>
              </div>
            </div>
          </div>
        </div>
        <div>
          <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:16px">
            ...
            <a href="riwayat-donor.html" class="text-sm text-primary font-semibold">Riwayat Donor →</a>
          </div>
  ...
  if (pageHeaderActions && document.querySelector('.welcome-card') && !isAdminPage) {
    if (loggedIn) {
      pageHeaderActions.innerHTML = `
        <a href="notifikasi.html" class="btn btn-ghost btn-sm" style="position:relative">
          🔔
        ...
        <a href="permintaan-darurat.html" class="btn btn-primary btn-sm">+ Permintaan Baru</a>`;
    }
  }
